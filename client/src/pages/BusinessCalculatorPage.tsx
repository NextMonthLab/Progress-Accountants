          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-600/50 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-slate-600/30">
              ...
              <Button
                ...
                onClick={() => {
                  // Generate detailed report with actual calculator data
                  const businessType = calculatorData.businessType.businessType.replace(/_/g, ' ').toUpperCase();
                  const industry = calculatorData.businessType.industry.replace(/_/g, ' ').toUpperCase();
                  
                  const reportContent = `BUSINESS ANALYSIS REPORT
========================

Generated: ${new Date().toLocaleDateString()}
Business: ${calculatorData.businessType.businessName || 'Your Business'}

BUSINESS PROFILE
================
Business Type: ${businessType}
Industry: ${industry}
Annual Revenue: £${calculatorData.financials.annualRevenue.toLocaleString()}
Monthly Expenses: £${calculatorData.financials.monthlyExpenses.toLocaleString()}
...
GROWTH PLANNING
===============
Revenue Growth Target: ${calculatorData.growth.revenueGrowthTarget.replace(/_/g, ' ')}
Planned Investment: ${calculatorData.growth.plannedInvestment.replace(/_/g, ' ')}
Planned Hires: ${calculatorData.growth.plannedHires}
${results.hiringImpact}

RECOMMENDED FOCUS AREAS
=======================
${results.recommendedAreas.map((area: string, index: number) => `${index + 1}. ${area}`).join('\n')}
...
NEXT STEPS
==========
Based on your business profile, consider these strategic actions:
...
---
Report generated by Progress Accountants Business Calculator
For professional advice tailored to your specific situation, contact Progress Accountants
...
Website: progressaccountants.co.uk`;
                  ...
                  toast({ title: "Download complete", description: "Your detailed business analysis report has been downloaded." });
                }}
              >
                <X className="h-6 w-6" />
              </Button>
            ...
            <div className="p-6">
              {leadCaptureEmbedCode ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-300 mb-6">
                    Please contact Progress Accountants directly to receive your business analysis report.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="p-6">
              {leadCaptureEmbedCode ? (
                ...
                <div className="text-center py-12">
                  <h4 className="text-xl font-semibold text-white mb-4">Form Not Available</h4>
                  <p className="text-slate-300 mb-6">
                    The lead form is currently unavailable. You can still download your report:
                  </p>
                  <Button
                    onClick={() => {
                      const businessType = calculatorData.businessType.businessType.replace('_', ' ').toUpperCase();
                      const industry = calculatorData.businessType.industry.replace('_', ' ').toUpperCase();
...
GROWTH PLANNING
===============
Revenue Growth Target: ${calculatorData.growth.revenueGrowthTarget.replace('_', ' ')}
Planned Investment: ${calculatorData.growth.plannedInvestment.replace('_', ' ')}
Planned Hires: ${calculatorData.growth.plannedHires}
${results.hiringImpact}

RECOMMENDED FOCUS AREAS
=======================
${results.recommendedAreas.map((area, index) => `${index + 1}. ${area}`).join('\n')}
...
---
Report generated by Progress Accountants Business Calculator
For professional advice, contact Progress Accountants
                      `.trim();
