...
const nameStepSchema = z.object({
  name: z.string().min(3, "Calculator name must be at least 3 characters"),
});
...
const displayStyleStepSchema = z.object({
  displayStyle: z.enum(['modal', 'card', 'full-page', 'widget']),
});

// Combined schema for the entire calculator
const calculatorSchema = nameStepSchema
...
const STEPS = [
  { id: 1, name: 'Name', description: 'Give your calculator a name' },
  { id: 2, name: 'Description', description: 'Describe what this calculator does' },
  { id: 3, name: 'Media', description: 'Add images or other media' },
  { id: 4, name: 'Display Style', description: 'Choose how the calculator will appear' },
  { id: 5, name: 'Confirm', description: 'Review and create' }
];

export default function CreateCalculatorWizard() {
  const { user } = useAuth();
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      const response = await apiRequest("POST", "/api/tools", {
        name: form.getValues('name') || "Untitled Calculator",
        description: form.getValues('description') || "Draft calculator",
        toolType: "calculator",
  ...
  const onSubmit = async (data: CalculatorValues) => {
    if (!user) {
      toast({
        title: "Authentication required",
        description: "Please sign in to create a calculator",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Your calculator has been created successfully",
        variant: "default",
      });
      queryClient.invalidateQueries(['/api/tools']);
      navigate('/tools-hub');
      ...
      toast({
        title: "Error",
        description: "Something went wrong while creating your calculator",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
  ...
  const handleMediaUpload = () => {
    ...
    form.setValue('mediaUrl', 'https://placehold.co/600x400/navy/white?text=Calculator+Image');
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Enter a name for your calculator" {...field} />
                </FormControl>
                <FormDescription>
                  Choose a descriptive name that explains what this calculator will do
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Describe what this calculator will calculate..."
                    className="min-h-[200px]"
                    {...field}
                  />
                </FormControl>
                <FormDescription>
                  Explain what this calculator will help users figure out
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Add an image that represents this calculator (optional)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="space-y-6">
                    ...
                    <FormItem>
                      <FormControl>
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="widget" className="flex flex-col">
                            <div className="font-medium">Widget</div>
                            <div className="text-sm text-muted-foreground">
                              Compact, embeddable calculator
                            </div>
                          </Label>
                        </div>
                      </FormControl>
                ...
                <FormDescription>
                  Select how you want your calculator to be displayed to users
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
        ...
        return (
          <div className="space-y-6">
            <div className="rounded-md border border-border p-4">
              <h3 className="font-medium mb-2">Calculator Summary</h3>
            ...
            <div className="text-sm text-muted-foreground">
              Please review the details above. When you're ready, click 'Create Calculator' to finalize your calculator.
            </div>
          </div>
        );
      default:
  ...
  return (
    <div className="container max-w-4xl mx-auto py-10">
      <Card className="border-2">
        <CardHeader>
          <CardTitle>Create Calculator Tool</CardTitle>
          <CardDescription>
            Create a custom calculator to provide valuable insights for your clients
          </CardDescription>
